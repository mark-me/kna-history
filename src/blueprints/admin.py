...
def validate_upload():
    ...
    try:
        ...
        if file_size > MAX_FILE_SIZE:
            return jsonify({
                'valid': False,
                'errors': [f'File too large. Max size: {MAX_FILE_SIZE / 1024 / 1024}MB']
            }), 400
...
def data_stats():
    ...
    try:
        ...
        # Performances count
        df = pd.read_sql("SELECT COUNT(*) as count FROM uitvoering WHERE type='Uitvoering'", 
                        con=reader.engine)
        ...
        # Media types count
        df = pd.read_sql("SELECT type_media, COUNT(*) as count FROM file GROUP BY type_media", 
                        con=reader.engine)
