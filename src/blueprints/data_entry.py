        ...
        with reader.engine.connect() as conn:
            stats = {
                "members": conn.execute(
                    text("SELECT COUNT(*) as count FROM lid")
                ).fetchone()[0],
                ...
                "roles": conn.execute(
                    text("SELECT COUNT(*) as count FROM rol")
                ).fetchone()[0],
                "files": conn.execute(
                    text("SELECT COUNT(*) as count FROM file")
                ).fetchone()[0],
            }
...
def delete_member(reader, id_lid):
    ...
    try:
        with reader.engine.connect() as conn:
            conn.execute(
                text("DELETE FROM lid WHERE id_lid = :id_lid"), {"id_lid": id_lid}
            )
            conn.commit()
...
def list_performances(reader):
    ...
    try:
        ...
        return render_template(
            "data_entry/performances_list.html", performances=performances
        )
    except Exception as e:
...
def edit_performance(reader, ref_uitvoering):
    ...
    try:
        ...
        return render_template(
            "data_entry/performance_form.html", performance=performance
        )
...
def delete_performance(reader, ref_uitvoering):
    ...
    try:
        with reader.engine.connect() as conn:
            conn.execute(
                text("DELETE FROM uitvoering WHERE ref_uitvoering = :ref_uitvoering"),
                {"ref_uitvoering": ref_uitvoering},
            )
            conn.commit()
...
@data_entry_bp.route(
    "/performances/<ref_uitvoering>/roles/<id_lid>/delete", methods=["POST"]
)
@login_required
@admin_required
@with_kna_reader  # Reader injected automatically!
    ...
    try:
        ...
        with reader.engine.connect() as conn:
            conn.execute(
                ...
                AND rol = :rol
            """),
                {"ref_uitvoering": ref_uitvoering, "id_lid": id_lid, "rol": rol},
            )
            conn.commit()
...
def upload_media(reader, ref_uitvoering):
    ...
    try:
        ...
        if not files or files[0].filename == "":
            ...
            return redirect(
                url_for("data_entry.manage_media", ref_uitvoering=ref_uitvoering)
            )
