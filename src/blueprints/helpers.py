...
def get_kna_config():
    ...
    """
    config = current_app.config.get('KNA_CONFIG')
    if not config:
...
def get_kna_reader():
    ...
    """
    reader = current_app.config.get('KNA_READER')
    if not reader:
...
def get_kna_loader():
    ...
    """
    loader = current_app.config.get('KNA_LOADER')
    if not loader:
...
def with_kna_reader(f):
    ...
    def decorated(*args, **kwargs):
        try:
            ...
            flash("Database niet beschikbaar. Neem contact op met de beheerder.", "danger")
            return redirect(url_for('home'))
