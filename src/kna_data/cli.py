...
def _validate_load_file(loader: KnaDataLoader, args) -> None:
    ...
    Runs validation on the provided Excel file and stops the load when blocking errors are found, while still reporting non-fatal warnings.
    """
...
def _execute_load(loader: KnaDataLoader, args) -> None:
    ...
    Executes the data load from the provided Excel file, logs a summary of loaded rows per table, and handles failures based on verbosity settings.
...
def _log_validation_success(validation: dict) -> None:
    ...
    Outputs summary information about validated sheets and any non-fatal warnings discovered during validation.
